...
def test_create_project_invalid_data(ProjectYouGile):
    invalid_project_data = { "invalid_field": "Some value" }
    response = ProjectYouGile.create_project(invalid_project_data)
    assert response.status_code == 400  # 400 Bad Request

def test_get_non_existing_project(ProjectYouGile):
    response = ProjectYouGile.get_project(77777)  # Предполагая, что такого проекта нет
    assert response.status_code == 404  # 404 Not Found

def test_update_non_existing_project(ProjectYouGile):
    project_id = 77777  # Убедимся, что проект не существует
    update_data = {
        "name": "Attempt to Update Non-existing Project",
    }
    response = ProjectYouGile.update_project(project_id, update_data)
    assert response.status_code == 404  # 404 Not Found
